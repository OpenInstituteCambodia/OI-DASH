...
package org.odk.collect.android.cfc.logic;
...
public class FormController {
    ...
    public int stepToPreviousScreenEventODK() throws JavaRosaException {
        try {
            if (getEvent() != FormEntryController.EVENT_BEGINNING_OF_FORM) {
                int event = stepToPreviousEvent();
    ...
    public FormEntryPrompt[] getQuestionPrompts() throws RuntimeException {
        ...
        if (element instanceof GroupDef) {
            ...
            for (int i = 0; i < indicies.size(); i++) {
                ...
                if (getEvent(index) == FormEntryController.EVENT_GROUP)
                    Log.i("seng", "EVENT_GROUP");
                if (getEvent(index) != FormEntryController.EVENT_QUESTION) {
                    String errorMsg =
